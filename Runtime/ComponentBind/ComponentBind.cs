...
namespace F8Framework.Core
{
    public class ComponentBind : MonoBehaviour
    {
...
#if UNITY_EDITOR
        private const string AutoGeneratedComponentMarker =
            "// Auto-generated component bindings below";
        private const string LegacyAutoGeneratedComponentMarker =
            "// 自动获取组件（自动生成，不能删除）";
        ...
        private void GenerateAutoBindComponentsCode(bool bindAllUIComponents)
        {
            ...
            // 使用正则表达式匹配并替换注释之间的内容。
            // 新模板使用英文标记，同时兼容旧模板中的中文标记。
            string markerPattern =
                $"(?:{System.Text.RegularExpressions.Regex.Escape(AutoGeneratedComponentMarker)}|" +
                $"{System.Text.RegularExpressions.Regex.Escape(LegacyAutoGeneratedComponentMarker)})";
            string pattern = $@"{markerPattern}(.*?){markerPattern}";
            ...
            {
                LogF8.Log($"在脚本中找不到插入标记。请在要生成代码的位置添加一对【{AutoGeneratedComponentMarker}】；旧脚本也兼容【{LegacyAutoGeneratedComponentMarker}】。");
            }
        }
