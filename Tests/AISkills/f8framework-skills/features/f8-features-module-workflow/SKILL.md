...
- The task is about creating custom modules, managing module lifecycle, or using StaticModule / ActivityModule.
...
Right-click in Project → F8 Module Center → Create Module / ModuleMono / StaticModule / ActivityModule template.
...
5. For auto-initialized global modules, use `StaticModule` pattern.
6. For activity gating and state-driven modules, use `ActivityModule` pattern.
7. Use right-click context menu to generate template files.
...
- Module type selected (Singleton / SingletonMono / StaticModule / ActivityModule).
